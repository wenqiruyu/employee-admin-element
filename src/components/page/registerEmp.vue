  ...
  <div style="margin-top: 40px;">
    ...
    <div style="margin-top: 40px;">
      <div v-if="active == 1" style="width: 600px; margin: 0 auto;">
        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="180px">
          ...
          <el-form-item label="入职时间" prop="participateTime">
            <el-date-picker
              v-model="ruleForm.participateTime"
              type="date"
              value-format="yyyy-MM-dd"
              v-bind:placeholder="ruleForm.participateTime"
      ...
      <div v-if="active == 2" style="width: 600px; margin: 0 auto;">
        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="180px">
          <el-form-item label="员工部门" prop="deptId" style="margin-left: 15px;">
          ...
          <el-form-item label="员工职位" prop="empPosition" style="margin-left: 15px;">
            <div style="float: left;width: 360px;">
          ...
          <el-form-item label="员工上级领导名" prop="superEmpName" style="margin-left: 15px">
            <el-input v-model="ruleForm.superEmpName" style="width: 222px;"></el-input>
          </el-form-item>
          <el-form-item label="员工上级领导员工号" prop="superEmpId" style="margin-left: 15px">
      ...
      <div v-if="active == 3" style="width: 600px; margin: 0 auto;">
        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="180px">
          <el-form-item label="平台用户名" style="margin-left: 15px;" prop="username">
            <el-input
              v-model="ruleForm.username"
              autocomplete="off"
              :disabled="true"
              style="width: 240px;"
            >{{this.ruleForm.username}}</el-input>
            <div style="margin-bottom: -30px;color: #727D81;font-size: 12px;">
              <span>*新用户用户名默认为员工姓名 + 员工号</span>
            </div>
          </el-form-item>
          <el-form-item label="平台用户密码" style="margin-left: 15px;" prop="password">
            <el-input
              type="password"
              ...
              :disabled="true"
              style="width: 240px;"
            >{{this.ruleForm.password}}</el-input>
            <div style="margin-bottom: -30px;color: #727D81;font-size: 12px;">
              <span>*新用户默认密码为Aa + 身份证后六位</span>
            </div>
          </el-form-item>
        ...
        <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="180px">
          <el-form-item label="紧急联系电话" prop="telephone" style="margin-left: 15px;">
            <el-input
              ...
              autocomplete="off"
              style="width: 222px;"
            >{{this.ruleForm.telephone}}</el-input>
          </el-form-item>
          <el-form-item label="详细户籍地址" prop="address" style="margin-left: 15px;">
            <el-input
              ...
              autocomplete="off"
              style="width: 222px;"
            >{{this.ruleForm.address}}</el-input>
          </el-form-item>
          <el-form-item label="现居住地址" prop="currentAddress" style="margin-left: 15px;">
            <el-input
              ...
              autocomplete="off"
              style="width: 222px;"
            >{{this.ruleForm.currentAddress}}</el-input>
          </el-form-item>
          <el-form-item label="所属党派" prop="politics" style="margin-left: 15px;">
            <div style="float: left;width: 360px;">
              <el-select v-model="ruleForm.politics" placeholder="选择所属党派">
                <el-option
                  v-for="(item,index) in politicsArr"
                  :key="item.value"
                  :value="item.value"
                  :label="item.label"
                ></el-option>
          ...
          <el-form-item label="所属民族" prop="national" style="margin-left: 15px;">
            <div style="float: left;width: 360px;">
          ...
          <el-form-item label="婚姻状态" prop="marriage" style="margin-left: 15px;">
            <div style="float: left;width: 360px;">
          ...
          <el-form-item label="个人学历" prop="education" style="margin-left: 15px;">
            <div style="float: left;width: 360px;">
...
export default {
  data() {
    ...
    const validateEmpName = (rule, value, callback) => {
      if (value === '') {
        return callback(new Error('请填写员工姓名'))
      } else {
        callback()
      }
    }
    ...
    const validateDeptId = (rule, value, callback) => {
      if (value == '' || value.length == 0) {
        callback(new Error('请填写员工部门'))
      } else {
        callback()
      }
    }
    const validateEmpPosition = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请填写员工职称'))
      } else {
        callback()
      }
    }
    const validateSuperEmpName = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请填员工上级领导名'))
      } else {
        callback()
      }
    }
    const validateSuperEmpId = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请填员工上级领导员工号'))
      } else {
        callback()
      }
    }
    ...
    const validateAddress = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请填员工详细户籍地址'))
      } else {
        callback()
      }
    }
    const validateCurrentAddress = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请填员工现居住地址'))
      } else {
        callback()
      }
    }
    ...
    return {
      ...
      rules: {
        ...
        county: [{ validator: validateCounty, trigger: 'blur' }],
        email: [{ validator: validateEmail, trigger: 'blur' }],
        participateTime: [
          { validator: validateParticipateTime, trigger: 'blur' }
        ],
        deptId: [{ validator: validateDeptId, trigger: 'blur' }],
        empPosition: [{ validator: validateEmpPosition, trigger: 'blur' }],
        superEmpName: [{ validator: validateSuperEmpName, trigger: 'blur' }],
        superEmpId: [{ validator: validateSuperEmpId, trigger: 'blur' }],
        sex: [{ validator: validateSex, trigger: 'blur' }],
        telephone: [{ validator: validateTelephone, trigger: 'blur' }],
        address: [{ validator: validateAddress, trigger: 'blur' }],
        currentAddress: [
          { validator: validateCurrentAddress, trigger: 'blur' }
        ],
        politics: [{ validator: validatePolitics, trigger: 'blur' }],
        national: [{ validator: validateNational, trigger: 'blur' }],
        marriage: [{ validator: validateMarriage, trigger: 'blur' }],
  ...
  methods: {
    ...
    getProvince() {
      this.$axios
        .get('http://localhost:22346/static/json/provinces.json')
        .then(res => {
    ...
    nextSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.active == 3 && this.ruleForm.email != '') {
            this.$axios
              .post('/employee-admin-server/user/getUserByEmail', {
              ...
              .then(res => {
                if (res.data.code == 200) {
                  if (res.data.data.length == 0) {
                    ...
                    this.$message.error('该邮箱已经注册过了哦，请更换邮箱')
                  }
                } else {
                  this.$message.error(res.data.msg)
                }
    ...
    submitForm(name) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          // 验证用户名是否存在
          this.$axios
            .post('/employee-admin-server/emp/addEmp', {
              params: {
                staffBase: {
